...
queueRoutes.get("/", async (c) => {
  ...
  const { data, error } = await sb
    .from("content_pieces")
    .select("id, status, channel_slug, format, content_payload, audit_notes, audit_category, audit_iterations, validation_result, similarity_score, final_character_count, created_at, image_url, video_url, media_status, variant_group_id, retry_count, retry_after, topic_id")
    .eq("workspace_id", ws)
